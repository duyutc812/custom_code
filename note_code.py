...
def check_manufacturer_import(self, convert, manufacturer, manufacturers_ext):
		...
		if manu_id:
			...
			if not value: 
				...
				supplier_id = self.import_data_connector(self.create_insert_query_connector('posts', update_supplier_posts_data))
				self.import_data_connector(self.create_update_query_connector('posts', {'guid': 'https://amvitamins.flywheelsites.com/?post_type=atum_supplier&#038;p=' + to_str(supplier_id)}, {'ID': supplier_id}))
				if supplier_id:
			...
			self.log(self.get_map_field_by_src(self.TYPE_MANUFACTURER, convert['id'], manu_id, field = 'value'), 'supplier_id.log')
...
def check_product_import(self, convert, product, products_ext):
	...
	if product_id:
		...
		check_product_atunm_exists = self.select_data_connector(self.create_select_query_connector('atum_product_data', {'product_id': product_id}))
		...
		if check_product_atunm_exists['data']:
			self.log(self.import_data_connector(self.create_update_query_connector('atum_product_data', {'supplier_id': supp_id, 'purchase_price': convert['cost_price']}, {'product_id': product_id})), 'get_supp_id')
		else:
			self.log(self.import_data_connector(self.create_insert_query_connector('atum_product_data', {'product_id': product_id, 'supplier_id': supp_id, 'purchase_price': convert['cost_price']})), 'get_supp_id')
		# self.log(convert['manufacturer']['id'], 'supp_id.log')
		# self.log(supp_id, 'supp_id.log')
		# self.log(product_id, 'product_id')
		self.log(self.import_data_connector(self.create_update_query_connector('postmeta', {'meta_value': convert['sku']}, {'post_id': product_id, 'meta_key': '_sku'})), 'update_sku')
		
		# update Compare at price - regular_price
		if convert.get('price'):
			check_product_regular_price_exists = self.select_data_connector(self.create_select_query_connector('postmeta', {'post_id': product_id, 'meta_key': '_regular_price'}))
			if check_product_regular_price_exists['data']:
				self.log(self.import_data_connector(self.create_update_query_connector('postmeta', {'meta_value': convert['price']}, {'post_id': product_id, 'meta_key': '_regular_price'})), 'update_regular_price')
			else:
				self.log(self.import_data_connector(self.create_insert_query_connector('postmeta', {'post_id': product_id, 'meta_key': '_regular_price', 'meta_value': convert['price']})), 'insert_regular_price')
		
		# update Price - sale price
		if convert['special_price'].get('price'):
			check_product_sale_price_exists = self.select_data_connector(self.create_select_query_connector('postmeta', {'post_id': product_id, 'meta_key': '_sale_price'}))
			check_product_price_exists = self.select_data_connector(self.create_select_query_connector('postmeta', {'post_id': product_id, 'meta_key': '_price'}))
			if check_product_sale_price_exists['data'] and check_product_price_exists['data']:
				self.log(self.import_data_connector(self.create_update_query_connector('postmeta', {'meta_value': convert['special_price'].get('price')}, {'post_id': product_id, 'meta_key': '_sale_price'})), 'update_sale_price')
				self.log(self.import_data_connector(self.create_update_query_connector('postmeta', {'meta_value': convert['special_price'].get('price')}, {'post_id': product_id, 'meta_key': '_price'})), 'update_price')
			else:
				self.log(self.import_data_connector(self.create_insert_query_connector('postmeta', {'post_id': product_id, 'meta_key': '_sale_price', 'meta_value': convert['special_price'].get('price')})), 'insert_sale_price')
				self.log(self.import_data_connector(self.create_insert_query_connector('postmeta', {'post_id': product_id, 'meta_key': '_price', 'meta_value': convert['special_price'].get('price')})), 'insert_price')

		#update barcode
		check_product_barcode_meta_exists = self.select_data_connector(self.create_select_query_connector('postmeta', {'post_id': product_id, 'meta_key': 'oliver_barcode'}))
		if check_product_barcode_meta_exists['data']:
			self.log(self.import_data_connector(self.create_update_query_connector('postmeta', {'meta_value': convert['barcode']}, {'post_id': product_id, 'meta_key': 'oliver_barcode'})), 'update_barcode')
		else:
			self.log(self.import_data_connector(self.create_insert_query_connector('postmeta', {'post_id': product_id, 'meta_key': 'oliver_barcode', 'meta_value': convert['barcode']})), 'insert_barcode')

		# update gtin
		check_product_gtin = self.select_data_connector(self.create_select_query_connector('postmeta', {'post_id': product_id, 'meta_key': '_rank_math_gtin_code'}))
		if check_product_gtin['data']:
			self.log(self.import_data_connector(self.create_update_query_connector('postmeta', {'meta_value': convert['barcode']}, {'post_id': product_id, 'meta_key': '_rank_math_gtin_code'})), 'update_gtin')
		else:
			self.log(self.import_data_connector(self.create_insert_query_connector('postmeta', {'post_id': product_id, 'meta_key': '_rank_math_gtin_code', 'meta_value': convert['barcode']})), 'insert_gtin')
		...
		self.log(self.import_data_connector(self.create_update_query_connector('postmeta', {'meta_value': convert['qty']}, {'post_id': product_id, 'meta_key': '_stock'})), 'update_stock_qty')
		self.log(self.import_data_connector(self.create_update_query_connector('postmeta', {'meta_value': stock_status}, {'post_id': product_id, 'meta_key': '_stock_status'})), 'update_stock_status')

		if convert.get('cost_price'):
			check_product_cost_meta_exists = self.select_data_connector(self.create_select_query_connector('postmeta', {'post_id': product_id, 'meta_key': 'product_cost'}))
			if check_product_cost_meta_exists['data']:
				self.log(self.import_data_connector(self.create_update_query_connector('postmeta', {'meta_value': convert['cost_price']}, {'post_id': product_id, 'meta_key': 'product_cost'})), 'update_cost')
			else:
				self.log(self.import_data_connector(self.create_insert_query_connector('postmeta', {'post_id': product_id, 'meta_key': 'product_cost', 'meta_value': convert['cost_price']})), 'insert_cost')
...
def check_product_import(self, convert, product, products_ext):
   ...
   if pro_id and convert['tier_prices']:
      ...
      for index, qty in enumerate(list_qty):
         ...
         self.log(self.api_v3('/catalog/products/' + to_str(pro_id) + '/bulk-pricing-rules', bulk_price, 'Post'), 'update_bulk_pricing_rules')
   ...
   if pro_id:
      ...
      if children_list:
         for key_child, product_child in enumerate(children_list):
            ...
            pro_child_id = self.get_map_field_by_src(self.TYPE_CHILD, product_child['id'], convert['code'], lang=self._notice['target']['language_default'])
            children_meta = {
            ...
            if check_pro_child_sale:
               ...
               for key, value in children_meta.items():
                  self.import_data_connector(
                     self.create_insert_query_connector('postmeta', {'post_id': pro_child_id, 'meta_key': key, 'meta_value': value}))
            if product_child['special_price']['price'] and (self.to_timestamp(product_child['special_price']['end_date']) > time.time() or (product_child['special_price']['end_date'] != '0000-00-00' or product_child['special_price']['end_date'] != '0000-00-00 00:00:00' or convert['special_price']['end_date'] == None)) or product_child['special_price']['end_date'] == '':
               ...
               if check_sale_date_from:
                  self.import_data_connector(self.create_update_query_connector('postmeta', {'meta_value': children_meta['_sale_price_dates_from']},
                                             {'post_id': pro_child_id, 'meta_key': '_sale_price_dates_from'}))
               else:
               ...
               if check_sale_date_to:
                  self.import_data_connector(self.create_update_query_connector('postmeta',
                                             {'meta_value': children_meta['_sale_price_dates_to']},
                                             {'post_id': pro_child_id,
                                             'meta_key': '_sale_price_dates_to'}))
               else:
                  self.import_data_connector(self.create_insert_query_connector('postmeta',
...
if cate_id:
   if len(category['path']) >= 6 and category['path'][:6] != '1/1285':
      ...
      collections_query = {
         'type': 'select',
         'query': "SELECT * FROM `_DBPRF_term_taxonomy` WHERE (taxonomy = 'product_cat' OR taxonomy = 'post_cat') AND term_id = " + to_str(cate_id)
      }
...
if product_id:
   ...
   if check_pro_attr:
      ...
      if pro_attr_data:
         ...
         if pro_attr_data.get('no-stock-message') and pro_attr_data.get('no-stock-message').get('value') == '[mgz_pagebuilder]{"elements":[]}[/mgz_pagebuilder]':
...
def display_import_source(self):
   ...
   self._notice['process']['products']['total_view'] = self._notice['process']['products']['total'] - self._notice['process']['products']['imported']
   ...
   self._notice['process']['categories']['total_view'] = self._notice['process']['categories']['total'] - self._notice['process']['categories']['imported']
...
if ord_items:
	for item in ord_items:
		...
		if to_decimal(item['price']) < 0 or to_decimal(item_data['quantity']) <= 0:
			continue
		product = self.select_map(self._migration_id, self.TYPE_PRODUCT, item['product']['id'], None, item['product']['code'])
		...
		if not product:
			...
			if pro_info_api:
				...
				if not pro_info:
					product_id = 0
				elif pro_info.get('inventory_tracking') == 'simple' and to_decimal(pro_info.get('inventory_level')) < to_decimal(item_data.get('quantity')):
					if product:
						value_product = json_decode(product['value'])
						if value_product and product_id:
							product_update = {
								'inventory_level': item['qty'] + to_int(value_product.get('qty')) if to_decimal(value_product.get('qty')) >= 0 else 0,

							}
							async_request.append(self.create_async_request_data('/products/' + to_str(product_id), product_update, 'PUT'))
					...
					if variants_info:
						...
						for var in variants:
							if to_str(var['sku']).strip() == src_sku.strip() and to_decimal(var.get('inventory_level')) < to_decimal(item_data.get('quantity')):
								product_update = {
									'inventory_level': to_int(item_data.get('quantity')) + to_int(var['inventory_level']),
								}
								if var['is_purchasing_disabled']:
									product_update['is_purchasing_disabled'] = False
									# update_purchase[product_id] = var['id']
								async_request.append(self.create_async_request_data('/products/' + to_str(product_id) + '/skus/' + to_str(var['id']), product_update, 'PUT'))
		if product_id:
		...
		if not item_sku or item_sku == '':
			if product:
				...
				item_sku = self.convert_attribute_code(item['product']['name']) if item['product']['name'] and item['product']['name'] != '' else self.convert_attribute_code(item['product']['code'])
		...
		if item['options'] and int(product_id) > 0:
			...
			if all_pro_opts_api or to_str(all_pro_opts_api) != '':
				...
				for item_option in item['options']:
					option = dict()
					option['id'] = get_row_value_from_list_by_field(all_pro_opts, 'display_name', item_option['option_name'], 'id')
					option_id = get_row_value_from_list_by_field(all_pro_opts, 'display_name', item_option['option_name'], 'option_id')
					...
					if len(list_attr_map_ids) > 1:
						...
						option['value'] = self.get_map_field_by_src(self.TYPE_ATTR_VALUE, None, item_option['option_value_name'].lower() + '-pro-' + to_str(product_id))
		...
		if all_pro_opts:
			for item_option in all_pro_opts:
				if item_option['is_required'] and not get_row_from_list_by_field(item_data['product_options'], 'id', item_option['id']):
					option = dict()
