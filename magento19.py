...
class LeCartMagento19(LeBasecart):
	...
	def get_query_display_import_source(self, update = False):
		...
		queries = {
			...
			'orders': {
				'type': 'select',
				'query': "SELECT COUNT(1) AS count FROM _DBPRF_sales_flat_order WHERE entity_id " + compare_condition + to_str(
	...
	def convert_category_export(self, category, categories_ext):
		...
		if category['parent_id'] and to_int(category['level']) > 2:
			if to_int(category['parent_id']) in self.cat_parent:
				self.log_primary('categories', 'parent', category['entity_id'])
				return response_error()
		...
		if 'include_in_menu' in eav_attribute:
		...
		for language_id, label in self._notice['src']['languages'].items():
			...
			name_lang = get_row_value_from_list_by_field(names, 'store_id', language_id, 'value')
			if not name_lang:
				name_lang = get_row_value_from_list_by_field(names, 'store_id', 0, 'value')
			description_lang = get_row_value_from_list_by_field(descriptions, 'store_id', language_id, 'value')
			if not description_lang:
				description_lang = get_row_value_from_list_by_field(descriptions, 'store_id', 0, 'value')
			meta_title_lang = get_row_value_from_list_by_field(meta_titles, 'store_id', language_id, 'value')
			if not meta_title_lang:
				meta_title_lang = get_row_value_from_list_by_field(meta_titles, 'store_id', 0, 'value')
			meta_keyword_lang = get_row_value_from_list_by_field(meta_keywords, 'store_id', language_id, 'value')
			if not meta_keyword_lang:
				meta_keyword_lang = get_row_value_from_list_by_field(meta_keywords, 'store_id', 0, 'value')
			meta_description_lang = get_row_value_from_list_by_field(meta_descriptions, 'store_id', language_id, 'value')
			if not meta_description_lang:
				meta_description_lang = get_row_value_from_list_by_field(meta_descriptions, 'store_id', 0, 'value')
			url_key_lang = get_row_value_from_list_by_field(url_key, 'store_id', language_id, 'value')
			if not url_key_lang:
				url_key_lang = get_row_value_from_list_by_field(url_key, 'store_id', 0, 'value')
	...
	def get_products_main_export(self):
		...
		query = {
			...
			'query': "SELECT * FROM _DBPRF_catalog_product_entity "
			         "WHERE entity_id NOT IN (SELECT product_id FROM _DBPRF_catalog_product_super_link WHERE parent_id NOT IN (SELECT entity_id FROM _DBPRF_catalog_product_entity where type_id ='grouped' )) AND entity_id in (5576) and entity_id > " + to_str(id_src) + " ORDER BY entity_id ASC LIMIT " + to_str(limit),
		}
	...
	def get_product_by_id(self, product_id):
		query = {
			'type': 'select',
			'query': "SELECT * FROM _DBPRF_catalog_product_entity "
			         "WHERE entity_id = " + to_str(product_id)
		}
		products = self.select_data_connector(query, 'products')
		if not products or products['result'] != 'success':
			return response_error()
	...
	def convert_product_export(self, product, products_ext):
		...
		product_data['barcode'] = get_row_value_from_list_by_field(entity_varchar, 'attribute_id', eav_attribute.get('barcode'), 'value')
		...
		country_of_manufacturer = get_row_value_from_list_by_field(entity_varchar, 'attribute_id', '117', 'value')
		commodity_code = get_row_value_from_list_by_field(entity_varchar, 'attribute_id', '220', 'value')
		product_data['country_of_manufacturer'] = country_of_manufacturer if country_of_manufacturer else None
		product_data['commodity_code'] = commodity_code if commodity_code else None
		...
		if to_len(product_images) > 0:
			...
			if check_img and check_img['disabled'] == '1':
				for product_image_thumb in product_images:
					...
					if product_data['thumb_image']['path'] or (product_image_thumb['value'] == 'no_selection' or (check_img and check_img['disabled'] == '1')):
						continue
					product_data['thumb_image']['url'] = url_product_image
		...
		if product_images:
			for product_image in product_images:
				...
				if check_img:
					product_image_data['status'] = False if to_int(check_img.get('disabled')) == 1 else True
		...
		for lang_id, lang_name in self._notice['src']['languages'].items():
			...
			if not short_description_lang:
				short_description_lang = get_row_value_from_list_by_field(short_descriptions, 'store_id', 0, 'value')
			...
			if not url_key_lang:
				url_key_lang = get_row_value_from_list_by_field(url_keys, 'store_id', 0, 'value')
			price_lang = get_row_value_from_list_by_field(prices, 'store_id', lang_id, 'value')
			if not price_lang:
				price_lang = get_row_value_from_list_by_field(prices, 'store_id', 0, 'value')
		...
		if product_options:
			for product_option in product_options:
				...
				option_data['option_type'] = product_option['type'] if product_option['type'] != 'field' else 'text'
				...
				for id, name in self._notice['src']['languages'].items():
					...
					if not option_name_lang:
						option_name_lang = get_row_value_from_list_by_field(option_title, 'store_id', 0, 'title')
				...
				for option_type_id in option_type_ids:
					...
					for id, name in self._notice['src']['languages'].items():
						...
						if not option_value_name_lang:
							option_value_name_lang = get_row_value_from_list_by_field(catalog_product_option_type_title, 'store_id', 0, 'title')
			...
			if children_products and super_attributes:
				for children in children_products:
					...
					if children_source:
						...
						children_weights = get_list_from_list_by_field(children_decimal, 'attribute_id', eav_attribute.get('weight'))
						...
						barcode_child = get_row_value_from_list_by_field(children_varchar, 'attribute_id', eav_attribute.get('barcode'), 'value')
						...
						if barcode_child:
							childen_data['barcode'] = barcode_child
						...
						for lang_id, lang_name in self._notice['src']['languages'].items():
							...
							if not childen_name_lang:
								childen_name_lang = get_row_value_from_list_by_field(children_names, 'store_id', 0, 'value')
							childen_description_lang = get_row_value_from_list_by_field(children_descriptions, 'store_id', lang_id, 'value')
							if not childen_description_lang:
								childen_description_lang = get_row_value_from_list_by_field(children_descriptions, 'store_id', 0, 'value')
							childen_short_description_lang = get_row_value_from_list_by_field(children_short_descriptions, 'store_id', lang_id, 'value')
							if not childen_short_description_lang:
								childen_short_description_lang = get_row_value_from_list_by_field(children_short_descriptions, 'store_id', 0, 'value')
							childen_meta_title_lang = get_row_value_from_list_by_field(children_meta_titles, 'store_id', lang_id, 'value')
							if not childen_meta_title_lang:
								childen_meta_title_lang = get_row_value_from_list_by_field(children_meta_titles, 'store_id', 0, 'value')
							childen_meta_keyword_lang = get_row_value_from_list_by_field(children_meta_keywords, 'store_id', lang_id, 'value')
							if not childen_meta_keyword_lang:
								childen_meta_keyword_lang = get_row_value_from_list_by_field(children_meta_keywords, 'store_id', 0, 'value')
							childen_meta_description_lang = get_row_value_from_list_by_field(children_meta_descriptions, 'store_id', lang_id, 'value')
							if not childen_meta_description_lang:
								childen_meta_description_lang = get_row_value_from_list_by_field(children_meta_descriptions, 'store_id', 0, 'value')
						...
						for super_attribute in super_attributes:
							...
							for id_lang, nameLang in self._notice['src']['languages'].items():
								...
								if not option_name_lang_sa:
									option_name_lang_sa = get_row_value_from_list_by_field(super_attribute_label, 'store_id', 0, 'value')
							...
							for id_lang, name_lang in self._notice['src']['languages'].items():
								...
								if not option_value_lang:
									option_value_lang = get_row_value_from_list_by_field(eav_attribute_option_value, 'store_id', 0, 'value')
		...
		for index, eav_attribute_row in enumerate(product_eav_attributes):
			...
			if eav_attribute_row['frontend_input'] == 'text' or eav_attribute_row['frontend_input'] == 'textarea':
				if eav_attribute_row['attribute_code'] == 'products_ebay_name':
					product_attribute_data['option_value_name'] = to_str(option_value_default['value'])
				# elif eav_attribute_row['attribute_code'] == 'url_key':
				# 	product_attribute_data['option_value_name'] = to_str(option_value_default['value'])
				else:
			...
			if eav_attribute_row['frontend_input'] == 'boolean':
				...
				if eav_attribute_row['attribute_id'] == '134':
					featured = 'Yes' if option_v_name == '1' else 'No'
					continue
				if eav_attribute_row['attribute_id'] == '206':
					exclue_from_sale = 'Yes' if option_v_name == '1' else 'No'
					continue
				product_attribute_data['option_value_name'] = 'Yes' if option_v_name == '1' else 'No'
			list_attr_not_metafields = ['material', 'colour', 'shisha_bowl_style', 'shisha_bowl_size', 'shisha_bowl_inside_finish',
										'shisha_bowl_compatible_with', 'shisha_pipe_style', 'shisha_pipe_size', 'shisha_pipe_no_of_hoses',
										'pack_size', 'shisha_flavour', 'nicotine_strength', 'base_product']
			if eav_attribute_row['attribute_code'] in list_attr_not_metafields:
				product_attribute_data['check'] = True
		...
		tagss = list()
		if featured:
			tagss.append('Featured_' + to_str(featured))
		if exclue_from_sale:
			tagss.append('Exclude From Sale_' + to_str(exclue_from_sale))

		product_data['tags'] += ','.join(tagss)
		...
		product_data['relate']['related_products'] = []
		product_data['relate']['up_sells'] = []
		product_data['relate']['cross_sells'] = []
		...
		if catalog_product_link_parent:
			for row in catalog_product_link_parent:
				...
				check_pro_id_exist = self.select_data_connector(self.create_select_query_connector('catalog_product_entity', {'entity_id': relation['id']}))
				if not check_pro_id_exist:
					continue
				relation['type'] = key
				if to_int(row['link_type_id']) == 1:
					product_data['relate']['related_products'].append(relation)
				elif to_int(row['link_type_id']) == 4:
					product_data['relate']['up_sells'].append(relation)
				else:
					product_data['relate']['cross_sells'].append(relation)
	...
	def channel_sync_inventory(self, product_id, convert, product, products_ext, settings = {}):
		...
		qty_setting = settings.get('price')
		product_eav_attribute = self.select_all_attribute_map()
		product_eav_attribute_data = dict()
		for attribute_data in product_eav_attribute:
			attribute = json.loads(attribute_data['value'])
		...
		def get_update_queryset(_product_id, _convert):
			all_queryset = list()
			if price_setting:
				data_attribute_insert = {
					'special_price': _convert.get('special_price', {}).get('price'),
					'special_from_date': _convert.get('special_price', {}).get('start_date'),
					'special_to_date': _convert.get('special_price', {}).get('end_date'),
					'price': to_decimal(_convert.get('price')),
				}
				for key, value in data_attribute_insert.items():
					if key not in product_eav_attribute_data:
						continue
					...
					all_queryset.append(self.create_update_query_connector('catalog_product_entity_' + product_eav_attribute_data[key]['backend_type'], product_attr_data, where))
			if qty_setting:
				update_qty = {
					'qty': _convert['qty'],
					'is_in_stock': 1 if (_convert['qty'] and _convert['manage_stock']) or not _convert['manage_stock'] or _convert.get('is_in_stock') else 0,
					'manage_stock': 1 if _convert['manage_stock'] else 0,
				}
				all_queryset.append(self.create_update_query_connector('cataloginventory_stock_item', update_qty, {'product_id': _product_id}))
			return all_queryset

		all_query = get_update_queryset(product_id, convert)
		if convert['children']:
			for children_convert in convert['children']:
				if self._notice['src']['cart_type'] == 'datasync':
					channel_id = self._notice['src']['config']['api']['channel_id']
					children = get_row_from_list_by_field(product['variants'], 'id', children_convert['id'])
					children_id = children['channel'].get(f'channel_{channel_id}', {}).get('product_id')
				else:
					children_id = self.get_map_field_by_src(self.TYPE_CHILD, children_convert['id'], children_convert['code'])
				if children_id:
					all_query += get_update_queryset(children_id, children_convert)
		if all_query:
	...
	def get_customers_ext_export(self, customers):
		...
		customer_ext_queries = {
			...
			'rewardpoints_customer': {
				'type': "select",
				'query': "SELECT * FROM _DBPRF_rewardpoints_customer WHERE customer_id IN " + customer_id_con
			}
		}
	...
	def convert_customer_export(self, customer, customers_ext):
		...
		# rewardpoints
		avai_points = get_row_value_from_list_by_field(customers_ext['data']['rewardpoints_customer'], 'customer_id', customer_data['id'], 'point_balance')
		hold_points = get_row_value_from_list_by_field(customers_ext['data']['rewardpoints_customer'], 'customer_id', customer_data['id'], 'holding_balance')
		spen_points = get_row_value_from_list_by_field(customers_ext['data']['rewardpoints_customer'], 'customer_id', customer_data['id'], 'spent_balance')
		customer_data['Available_Points_Balance'] = avai_points if avai_points else '0'
		customer_data['On_Hold_Points_Balance'] = hold_points if hold_points else '0'
		customer_data['Spent_Points'] = spen_points if spen_points else '0'
	...
	def get_orders_main_export(self):
		...
		query = {
			'type': 'select',
			'query': "SELECT * FROM _DBPRF_sales_flat_order WHERE state != 'canceled' and entity_id > " + to_str(id_src) + " ORDER BY entity_id ASC LIMIT " + to_str(limit),
		}
	...
	def get_orders_by_created_at(self, created_at):
		...
		query = {
			'type': 'select',
			'query': "SELECT * FROM _DBPRF_sales_flat_order WHERE " + self.get_con_store_select_count() + f" AND created_at >= '{created_at}' AND entity_id > " + to_str(id_src) + " ORDER BY entity_id ASC LIMIT " + to_str(limit),
		}
		orders = self.select_data_connector(query, 'orders')
	...
	def convert_order_export(self, order, orders_ext):
		...
		payment_method = {
			'sagepayserver': 'Secure Credit / Debit Card Payments',
			'm2epropayment': 'm2Epro',
			'paypal_express': 'PayPal Express',
			'multipaymentforpos': 'Multiple Payments',
			'sagepaydirectpro': 'Credit Card (Sage Pay)',
			'free': 'No Payment Info',
			'banktransfer': 'Bank Transfer Payment',
			'cashforpos': 'Cash In',
			'ccforpos': 'CC',
			'cp1forpos': 'Custom payment 1',
			'hosted_pro': 'Secure Credit / Debit Card Payments',
			'sagepayserver_moto': 'Secure Credit / Debit Card Payments'
		}
		order_payment_method = payment_method.get(order_payment['title'])
		order_data['payment_method'] = order_payment_method if order_payment_method else None

		base_total_paid = to_decimal(order.get('base_total_paid'))
		total_paid = to_decimal(order.get('total_paid'))
		# total_paid = order.get('total_paid')
		paid_total = to_str(base_total_paid) if base_total_paid == total_paid else (to_str(base_total_paid) + '[' + to_str(total_paid) + ']')

		order_data['paid'] = 'Paid: ' + paid_total
		order_data['tags'] = order_data['payment_method']
	...
	def convert_coupon_export(self, coupon, coupons_ext):
		...
		if not salesrule_coupon:
			return response_error(coupon['rule_id'] + 'coupon code empty')
