...
class LeCartShopify(LeBasecart):
	...
	def display_config_target(self):
		...
		try:
			...
			response['elm'] = '#error-api'
			return response
		created_at = shop.get('shop', dict()).get('time_zone')
	...
	def check_product_import(self, convert, product, products_ext):
		...
		# 			self.log(f"{convert['id']} | {convert['sku']} |{product_id} | {pro_related_metafield}", 'related_products')
	...
	def product_import(self, convert, product, products_ext):
		...
		if convert['attributes']:
			for attribute in convert['attributes']:
				if attribute.get('check'):
					if product.get('attribute_set_id') == '13' and attribute['option_code'] not in ['pack_size',
																									'shisha_flavour',
																									'nicotine_strength',
																									'base_product']:
						continue
					if ';' in attribute['option_value_name']:
						tagggs.extend([attribute['option_name'] + '_' + x for x in attribute['option_value_name'].split(';')])
					else:
						if attribute['option_value_name']:
							tagggs.append(attribute['option_name'] + '_' + attribute['option_value_name'])
					continue
				if to_str(attribute['option_name']) == 'Description' or to_str(attribute['option_name']) == 'Short Description' or not attribute['option_name'] or attribute['option_code'] == 'commodity_code':
					continue
		...
		inventory_item_id = var_response.get('variant').get('inventory_item_id')
		if inventory_item_id:
			if convert['country_of_manufacturer'] or convert['commodity_code']:
				update_inventory = {
					"inventory_item": {
						"country_code_of_origin": convert['country_of_manufacturer'],
						"province_code_of_origin": None,
						"harmonized_system_code": convert['commodity_code'],
					}
				}
				self.api('/inventory_items/' + to_str(inventory_item_id) + '.json', update_inventory, 'PUT')
	...
	def customer_import(self, convert, customer, customers_ext):
		...
		update_customer_1 = {
			"metafield": {
				"namespace": "global",
				"key": "available_points_balance",
				"value": convert['Available_Points_Balance'] + ' Points' if to_int(convert['Available_Points_Balance']) > 1 else convert['Available_Points_Balance'] + ' Point',
				"value_type": "string",
			}
		}
		self.api('customers/' + to_str(id_desc) + '/metafields.json', update_customer_1, 'Post')
		update_customer_2 = {
			"metafield": {
				"namespace": "global",
				"key": "on_hold_points_balance",
				"value": convert['On_Hold_Points_Balance'] + ' Points' if to_int(
					convert['On_Hold_Points_Balance']) > 1 else convert['On_Hold_Points_Balance'] + ' Point',
				"value_type": "string",
			}
		}
		self.api('customers/' + to_str(id_desc) + '/metafields.json', update_customer_2, 'Post')
		update_customer_3 = {
			"metafield": {
				"namespace": "global",
				"key": "spent_points",
				"value": convert['Spent_Points'] + ' Points' if to_int(
					convert['Spent_Points']) > 1 else convert['Spent_Points'] + ' Point',
				"value_type": "string",
			}
		}
		self.api('customers/' + to_str(id_desc) + '/metafields.json', update_customer_3, 'Post')
	...
	def get_sizes(self, url):
		req = requests.get(url, headers = {'User-Agent': self.USER_AGENT, 'Range': 'bytes'})
		try:
